...
type Props = {
  ...
  showNumbers?: boolean;  // mostra fileira 1-0 acima do QWERTY (lições com dígitos)
};
 ...
 * Quando `showNumbers`, renderiza a fileira numérica acima do QWERTY.
 */
export function Keyboard({ highlight, lastPressed, showFingers = true, compact = false, showNumbers = false }: Props) {
  ...
  return (
    <div className="select-none w-full max-w-3xl mx-auto p-2 md:p-4 rounded-3xl bg-white/60 backdrop-blur shadow-bubbly">
      {showNumbers && (
        <div className="flex justify-center gap-1 md:gap-1.5 my-1">
          {NUMBER_ROW.map(renderKey)}
        </div>
      )}
      {KEY_ROWS.map((row, ri) => (
